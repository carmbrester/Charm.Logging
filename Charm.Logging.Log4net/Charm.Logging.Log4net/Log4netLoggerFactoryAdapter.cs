...
/*
 * Copyright © Chris Armbrester
 *
 * Added configuration for the ambient context.
 */
...
namespace Charm.Logging.Log4net
{
    ...
    public class Log4netLoggerFactoryAdapter : ILoggerFactoryAdapter
    {
        ...
        {
            return new LoggerAdapter(_runtime.GetLogger(name), _executionIdProvider, _circumventLogicalThreadContext);
        }

        public ILogger GetLogger(Type type)
        {
            var logger = _runtime.GetLogger(type);
            return new LoggerAdapter(logger, _executionIdProvider, _circumventLogicalThreadContext);
        }
    }
}
