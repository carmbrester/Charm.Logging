 ...
 *
 * Nearly all of this implementation comes from Github user "illearth".
 * Changes here are the addition of the exception to be relayed on to the recipient when
 * the first appender fails.
 */
...
namespace Charm.Logging.Log4net.Appenders
{
    /// <summary>
    ...
    public class AsyncRolloverAppender : ForwardingAppender
    {
        ...
        private Exception _appenderErrorException;
        ...
        /// <summary>
        /// If greateer than zero, number of seconds before resetting to the first appender.
                        ...
                        if (_logEvents.TryDequeue(out levent))
                        {
                            ...
                            while (levent != null && appenderIndex < Appenders.Count)
                            {
                                try
                                {
                                    ...
                                    if (AppenderError())
                                    {
                                        appenderIndex = NextAppender(appenderIndex, _appenderErrorException);
                                    }
                                    else
                                    {
                                        levent = null;
        ...
        private void NotifyOfChangedAppender(int failedAppenderIndex, Exception ex = null)
        {
            if (!String.IsNullOrWhiteSpace(RolloverNotificationLoggerName))
            {
                ...
                var logger = log4net.LogManager.GetLogger(RolloverNotificationLoggerName);
    ...
    internal class RolloverErrorHandler : IErrorHandler
    {
        ...
        private void Notify(Exception ex = null)
        {
            if (!_errorOccured)
            {
                _rolloverAppender.AppenderError(_appender, ex);
                _errorOccured = true;
            }
        }
    }
