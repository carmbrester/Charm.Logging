 ...
 *
 * Removed pre-processor compilation conditionals and changed namespace.
 */
...
namespace Charm.Logging
{
    ...
    public static class ArgUtils
    {
        ...
        public static T TryParseEnum<T>(T defaultValue, string stringValue) where T : struct
        {
            ...
            {
                Trace.WriteLine(string.Format("WARN: failed converting value '{0}' to enum type '{1}'", stringValue,
        ...
        public static T TryParse<T>(T defaultValue, string stringValue)
        {
            ...
            {
                throw new ArgumentException(string.Format("There is no parser registered for type {0}",
                    typeof (T).FullName));
            }
            ...
            {
                Trace.WriteLine(
                    string.Format("WARN: failed converting value '{0}' to type '{1}' - returning default '{2}'",
        ...
        /// Throws a <see cref="ArgumentNullException"/> if <paramref key="val"/> is <c>null</c>.
        /// </summary>
        public static T AssertNotNull<T>(string paramName, T val, string messageFormat, params object[] args)
            where T : class
        {
            if (ReferenceEquals(val, null))
            {
        ...
        /// assignable to type <typeparam key="T"></typeparam>.
        /// </summary>
        public static Type AssertIsAssignable<T>(string paramName, Type valType, string messageFormat,
            params object[] args)
        {
            if (valType == null)
            {
